...
import (
	...
	//"github.com/kubernetes/kubernetes/pkg/util/strings"
)
...
func (p *progress) updateServer(source string) {
	if glog.V(6) { glog.Infof("%s*** Updating Turbo (%s): %d%% %s", p.logPrefix, source, p.pct, p.desc) }
...
func (p *progress) KeepAlive(interval time.Duration, stop chan struct{}) {
	...
	//	select {
	//	case <-stop:
	//		// stop the timer (and drain channel if necessary to prevent it firing)
	//		if !t.Stop() {
	//			<-t.C
	//		}
	//
	...
	//		return
	//	case <-t.C:
	//		glog.Infof("%sProgress: *** keepAlive goroutine updating", p.logPrefix)
...
//
// TODO: For integration with kubeturbo:
// TODO: 1. Modify the kubeturbo TurboActionExecutor.Execute() method by adding progress (NOT tracker) to input args
// TODO: 2. Remove this stub interface code for TurboActionExecutor interface and use the kubeturbo version
//
type TurboActionExecutor interface {
	//Execute(input *TurboActionExecutorInput) (*TurboActionExecutorOutput, error)
	//Execute(input string) (string, error)
	//Execute(input string, progress string) (string, error)
	//Execute(input string, progress *progress) (string, error)
	Execute(input string) (string, error)
}

type TurboNodeActionExecutor interface {
	TurboActionExecutor
	//ExecuteWithProgress(input *TurboActionExecutorInput, progress *progress) (*TurboActionExecutorOutput, error)
	ExecuteWithProgress(input string, progress *progress, lockKey string) (string, error)
}
...
type ScaleActionExecutor interface {
	...
	parseInput(string, *progress) (*scalingRequest, error)
	...
	verifyActionSucceeded(ScalingController) (string, error)
...
func (s *scaler) Execute(vmDTO string) (string, error) {
...
func (s *Provisioner) Execute(vmDTO string) (string, error) {
	// create progress indicator
	// TODO: Use actual Turbo tracker object when integrated
	tracker := "replace this string with tracker object when integrated"
	progress := NewProgress(tracker)
...
func (s *Suspender) Execute(vmDTO string) (string, error) {
	// create progress indicator
	// TODO: Use actual Turbo tracker object when integrated
	tracker := "replace this string with tracker object when integrated"
	progress := NewProgress(tracker)
...
func (s *scaler) ExecuteWithProgress(vmDTO string, progress *progress, lockKey string) (string, error) {
	return ScaleNode(s, vmDTO, progress, lockKey)
}

//func (s *Provisioner) Execute(vmDTO string, tracker string) (string, error) {
func (s *Provisioner) ExecuteWithProgress(vmDTO string, progress *progress, lockKey string) (string, error) {
	return ScaleNode(s, vmDTO, progress, lockKey)
}

//func (s *Suspender) Execute(vmDTO string, tracker string) (string, error) {
func (s *Suspender) ExecuteWithProgress(vmDTO string, progress *progress, lockKey string) (string, error) {
	return ScaleNode(s, vmDTO, progress, lockKey)
}

// getLockKey returns a key suitable for locking a scaling action to the specific ScalingController that manages a Node.
func GetLockKey(s ScaleActionExecutor, actionItemDTO string, progress *progress) (string, error) {
...
func (s *scaler) parseInput(dto string, progress *progress) (*scalingRequest, error) {
	// parse the ActionItemDTO to identify the Node name and number of Nodes to scale
	nodeName, err := s.identifyNodeName(dto)
	if err != nil {
		return nil, fmt.Errorf("cannot identify Node to scale: %v", err)
	}

	// TODO(future): Parse the DTO for the number of Nodes to scale.
	// TODO(future): In the meantime, assume all scaling request are for only a single Node.
	numNodes := int32(1)
...
func ScaleNode(s ScaleActionExecutor, vmDTO string, progress *progress, lockKey string) (string, error) {
	// process action and log any terminating error encountered
	result, err := scaleNode(s, vmDTO, progress, lockKey)
	//result, err := scaleNode(sController)
...
func scaleNode(s ScaleActionExecutor, vmDTO string, progress *progress, lockKey string) (string, error) {
//func scaleNode(sc ScalingController) (string, error) {
	//s.progressIndicator().update(0, "", "", "scaling action")

	// parse input and get a ScalingRequest
	progress.update(0, "Identifying scaling controller managing the Node", ActionIdentification)
	sr, err := s.parseInput(vmDTO, progress)
	if err != nil {
		return "", &ActionError{ActionIdentification, err}
	}
	...
	sc.Progress().update(10, "Verifying scaling action preconditions", ActionPrecondition)
	err = s.verifyActionPreconditions(sc)
	//err := sc.verifyActionPreconditions()
	if (err != nil) {
		return "", &ActionError{ActionPrecondition, err}
	}

	// execute scaling action
	sc.Progress().update(20, "Executing scaling action", ActionExecution)
	err = s.executeAction(sc)
	//err = sc.executeAction()
	if (err != nil) {
		return "", &ActionError{ActionExecution, err}
	}

	// wait for scaling action to complete
	sc.Progress().update(60, "Verifying scaling action postconditions", ActionPostcondition)
	result, err := s.verifyActionSucceeded(sc)
	//result, err := sc.verifyActionSucceeded()
	if (err != nil) {
		return "", &ActionError{ActionPostcondition, err}
	}

	sc.Progress().update(100, result, ActionSucceeded)
	return result, nil
}
...
type Controller interface {
	...
	verifyActionSucceeded() (string, error)
...
func (sc *scalingController) verifyActionSucceeded() (string, error) {
	return "", fmt.Errorf("verifyActionSucceeded: scaling support not yet implemented controller type %T: %v", sc, sc)
}
...
func (sc *msScalingController) verifyActionSucceeded() (string, error) {
	ms := sc.ms
	mList := sc.mList
	logPrefix := sc.logPrefix
	...
	//// override mList for testing
	//newMList = mList.DeepCopy()
	////newMList.Items = newMList.Items[0:1] // set slice ([]Machine) to only first element
	//newMList.Items = newMList.Items[0:0] // set slice ([]Machine) to empty slice
	//msg := fmt.Sprintf("newMList.Items(%d)= %v", len(newMList.Items), newMList.Items)
	//glog.Info(logPrefix + msg)
	//
	//////replicas := *ms.Spec.Replicas
	//////msg := fmt.Sprintf("#Machines(%d) == new replica count(%d) for MachineSet %s", len(newMList.Items), replicas, ms.Name)
	////glog.Info(logPrefix + msg)
	// TODO: ***** Remove test code
	...
	case ProvisionAction:
		...
		if err != nil {
			return "", fmt.Errorf("Machine %s failed to provision new Node in MachineSet %s: %v", newM.Name, ms.Name, err)
		}
	case SuspendAction:
		// wait for MachineSet to stabilize, i.e., when number of Machines equals the replica count
		replicas := *ms.Spec.Replicas
		...
		if err != nil {
			return "", fmt.Errorf("Machine %s failed to deprovision Node %s in MachineSet %s: %v", deletedM.Name, deletedM.Status.NodeRef.Name, ms.Name, err)
		}
	default:
		return "", fmt.Errorf("Unexpected MachineSet scaling action type: %v", actionType)
	}
...
func (sc *msScalingController) waitForNodeProvisioning(newM *clusterv1.Machine) (string, error) {
	logPrefix := sc.logPrefix

	if glog.V(3) { glog.Infof(logPrefix + "Identifying Node created by Machine %s", newM.Name) }

	// wait for Machine's status to indicate Node creation final status
	stateDesc := fmt.Sprintf("Machine %s Node creation status is final", newM.Name)
	err := sc.waitForState(stateDesc, nodeCreatedMaxWaits, nodeCreatedWaitTime, 70, 90, sc.isNodeSuccessOrError, newM.Name)
	if err != nil {
		return "", err
	}
	...
	if glog.V(2) { glog.Infof(logPrefix + "Identified %s as Node created by Machine %s", newNName, newM.Name) }

	// wait for new Node to be in Ready state
	stateDesc = fmt.Sprintf("Node %s is Ready", newNName)
	err = sc.waitForState(stateDesc, nodeReadyMaxWaits, nodeReadyWaitTime, 90, 100, sc.isNodeReady, newNName)
	if err != nil {
		return "", err
	}
...
func (sc *msScalingController) waitForNodeDeprovisioning(deletedM *clusterv1.Machine) (string, error) {
	logPrefix := sc.logPrefix

	if glog.V(3) { glog.Infof(logPrefix + "Identifying Node managed by deleted Machine %s", deletedM.Name) }
	...
	stateDesc := fmt.Sprintf("Node %s deleted or exited Ready state", deletedNName)
	err := sc.waitForState(stateDesc, nodeNotReadyMaxWaits, nodeNotReadyWaitTime, 70,100, sc.isNodeDeletedOrNotReady, deletedNName)
	if err != nil {
		return "", err
	}

	// TODO: Consider deleting the Node since Cluster API does not currently delete it and Core Kubernetes only
	// TODO: marks it as Not Ready since it is not responding.
...
func (sc *msScalingController) executeAction() error {
	...
	sc.progress.update(20, "Updating " + updateMsg)
...
func (s *Provisioner) verifyActionSucceeded(sc ScalingController) (string, error) {
...
func (s *Suspender) verifyActionSucceeded(sc ScalingController) (string, error) {
	return sc.verifyActionSucceeded()
}
...
func (s *scaler) verifyActionSucceeded(sc ScalingController) (string, error) {
	return sc.verifyActionSucceeded()
}
...
func (sc *scalingController) waitForState(stateDesc string, maxWaits int, waitTime time.Duration, progStart, progEnd int, f isState, args ...interface{}) error {
	...
	numWaits := 0
	for {
		progress.update(progStart + int(progInc * float32(numWaits)), "Verifying " + stateDesc)

		// call function to check state
		b, fErr := f(logPrefix, args...)
		if fErr == nil && b {
			if glog.V(2) {
				glog.Infof(logPrefix + "Verified %s (wait=%v)",
					stateDesc, time.Duration(numWaits) * waitTime)
			}
			return nil
		}
...
func (c *apiClients) verifyClusterAPIEnabled(groupVersion, logPrefix string) error {
	...
	if glog.V(3) { glog.Infof(logPrefix + "Verifying %s is available", serviceString) }
	...
	if glog.V(2) { glog.Infof(logPrefix + "Verified %s is available", serviceString) }
	return nil
}
...
func (s *apiClients) getObjectWithLogging(logPrefix string, name string, objectType string, f getter) (interface{}, error) {
	if glog.V(5) { glog.Infof(logPrefix + "Retrieving %s %s", objectType, name) }
...
func (s *apiClients) listObjectsWithLogging(logPrefix string, name interface{}, objectType string, f lister) (lengther, error) {
	if glog.V(5) { glog.Infof(logPrefix + "Retrieving %s", objectType) }
...
func (s *apiClients) updateMachineSet(ms *clusterv1.MachineSet, updateMsg, logPrefix string) error {
	if glog.V(5) { glog.Infof(logPrefix + "Updating " + updateMsg) }
	...
	if glog.V(4) { glog.Infof(logPrefix + "Updated %s", updateMsg) }
	return nil
}
...
// identifyNodeName returns the name of the Node that corresponds to a Turbo VM EntityDTO.  The Node name is located in
// an EntityDTO property.  An error is returned if the EntityDTO property is not found.
//
// Example: The Node name "machineset-small-867bj" is returned for the following VM EntityDTO:
//   entityType:VIRTUAL_MACHINE
//     id:"05fbc0d8-bcd8-11e8-a9af-42010a800002" displayName:"machineset-small-8ndvx”
//     entityProperties:<namespace:"DEFAULT" name:"KubernetesNodeName" value:"machineset-small-8ndvx" >
func (h *scaler) identifyNodeName(vmDTO string) (string, error) {
	//// find Node name in property list
	//vmProperties := vmDTO.GetEntityProperties()
	//for _, vmProperty := range vmProperties {
	//	if *vmProperty.Name == vmDTOPropK8sNodeName {
	//		return *vmProperty.Value, nil
	//	}
	//}
	//
	//err := fmt.Errorf("\"%s\" property not found on VM EntityDTO %s", vmDTOPropK8sNodeName, vmDTO.GetDisplayName())
	//
	//// if Node name not found in property list, use the VM display name
	//vmName := vmDTO.GetDisplayName()
	//if vmName != "" {
	//	glog.Warningf("%s, using VM DisplayName instead: %v", err, vmName)
	//	return vmName, nil
	//}
	//
	//return "", err
...
func (c *apiClients) identifyManagingMachine(nodeName, msg, logPrefix string) (*clusterv1.Machine, error) {
	if glog.V(3) { glog.Infof(logPrefix + "Identifying %s", msg) }
	...
	if glog.V(2) { glog.Infof(logPrefix + "Identified %s as %s", machineName, msg) }
	return m, nil
}
...
func (s *apiClients) identifyManagingMachineSet(machineName, msg, logPrefix string) (*clusterv1.MachineSet, *clusterv1.MachineList, error) {
	...
	for _, ms = range msList.Items {
		...
		for _, m := range mList.Items {
			if glog.V(6) { glog.Infof(logPrefix + "Comparing Machine names %s == %s: %v", machineName, m.Name, machineName==m.Name) }
			if machineName == m.Name {
				msFound = true
				if glog.V(6) { glog.Infof(logPrefix + "Found Machine %s in MachineSet %s", machineName, ms.Name) }
				break FindMSLoop
			}
		}
	}
	...
	if glog.V(2) { glog.Infof(logPrefix + "Identified %s as %s", ms.Name, msg) }
	return &ms, mList, nil
}
...
func (sc *scalingController) identifyExtraMachine(mList1 *clusterv1.MachineList, mList2 *clusterv1.MachineList, msg, logPrefix string) (*clusterv1.Machine, error) {
	...
	if glog.V(2) { glog.Infof(logPrefix + "Identified %s as %s", newM.Name, msg) }
	return newM, nil
}
...
func roleContains(a common.MachineRole, list []common.MachineRole) bool {
	for _, b := range list {
		if b == a {
			return true
		}
...
// isMaster returns true if the machine has the Master role; false, otherwise.
func isMaster(machine *clusterv1.Machine) bool {
	return roleContains(common.MasterRole, machine.Spec.Roles)
}
